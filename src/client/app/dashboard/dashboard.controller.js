(function () {
  ...
  DashboardController.$inject = ['$q', '$rootScope', '$scope', 'dataservice', 'logger'];
  /* @ngInject */
  function DashboardController($q, $rootScope, $scope, dataservice, logger) {
    var client;
    ...
    $scope.$on('evilMode', function (event, data) {
      vm.evilMode =data;
    });

    vm.toggleEvil = function () {

      vm.evilMode = !vm.evilMode;

      $scope.$emit('evilMode', vm.evilMode);
    ...
    //connect('test.mosquitto.org', 8080, 'elcosito-client');
    connect('192.168.1.4', 3000, 'elcosito-client');
